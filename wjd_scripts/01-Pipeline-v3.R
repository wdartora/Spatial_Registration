...
output_fig_dir <- "r_output3/"
output_r_dir <- "r_output3/"
...
visium_path <- "Y:/William/Data_base_VISIUM/MPI/data/WTP-5-a7-8_PICALM-5-a1-8/outs"
...
se1 <- subset(se,  x < 8500, invert =F)
se2 <- subset(se,  x < 9500, invert =T)
...
sct_transform(se1, "WTP-5-a7-8")
sct_transform(se2, "PICALM-5-a1-")

#####Merge all objects together and re-run pipeline all together

se <- readRDS(paste0(output_r_dir, "WTP-5-a7-8",".rds"))
paths <- c("WTP-5-a7-8","PICALM-5-a1-")
...
img <- readImage("Y:/William/Data_base_VISIUM/MPI/data/WTP-5-a7-8_PICALM-5-a1-8/outs/spatial/tissue_hires_image.png")
...
max(coords_sub$imagerow[grepl("WTP-5-a7-8",coords_sub$slice)])
min(coords_sub$imagerow[grepl("PICALM-5-a1-",coords_sub$slice)])

# Split the image literally  ////////////////
bottom_cutoff <- 1950 # 1950
top_cutoff <- 1925 # NOTE DIFFERENT NUMBER 2200


img1 <- img[,1:bottom_cutoff,]
img2 <- img[,top_cutoff:ncol(img),]

png(paste0(output_fig_dir,"WTP-5-a7-8_hires_image.png"), height=dim(img1)[2],width=dim(img1)[1],res=800)
display(img1, method="raster")
coords_sub2 <- coords_sub %>% filter(slice  == "WTP-5-a7-8")
#points(coords_sub2$imagecol,coords_sub2$imagerow,pch=3,cex=.1)
dev.off()

png(paste0(output_fig_dir,"PICALM-5-a1-_hires_image.png"), height=dim(img2)[2],width=dim(img2)[1],res=800)
display(img2, method="raster")
coords_sub2 <- coords_sub %>% filter(slice == "PICALM-5-a1-")
...
coords_spots <- coords_sub %>%
  ...
  mutate(imagerow = ifelse(slice == "PICALM-5-a1-", imagerow-top_cutoff, imagerow)) 
