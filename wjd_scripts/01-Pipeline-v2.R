...
output_fig_dir <- "r_output2/"
output_r_dir <- "r_output2/"
...
visium_path <- "Y:/William/Data_base_VISIUM/MPI/data/ABCA7-8-a3_BIN1-5-a1/outs"
...
sct_transform(se1, "ABCA7-8-a3")
sct_transform(se2, "BIN1-5-a1")

#####Merge all objects together and re-run pipeline all together

se <- readRDS(paste0(output_r_dir, "ABCA7-8-a3",".rds"))
paths <- c("ABCA7-8-a3","BIN1-5-a1")
...
img <- readImage("Y:/William/Data_base_VISIUM/MPI/data/ABCA7-8-a3_BIN1-5-a1/outs/spatial/tissue_hires_image.png")
...
max(coords_sub$imagerow[grepl("ABCA7-8-a3",coords_sub$slice)])
min(coords_sub$imagerow[grepl("BIN1-5-a1",coords_sub$slice)])

# Split the image literally  ////////////////
bottom_cutoff <- 2250 # 975
top_cutoff <- 2200 # NOTE DIFFERENT NUMBER 990

img1 <- img[,1:bottom_cutoff,]
img2 <- img[,top_cutoff:ncol(img),]

png(paste0(output_fig_dir,"ABCA7-8-a3_hires_image.png"), height=dim(img1)[2],width=dim(img1)[1],res=800)
display(img1, method="raster")
coords_sub2 <- coords_sub %>% filter(slice  == "ABCA7-8-a3")
#points(coords_sub2$imagecol,coords_sub2$imagerow,pch=3,cex=.1)
dev.off()

png(paste0(output_fig_dir,"BIN1-5-a1_hires_image.png"), height=dim(img2)[2],width=dim(img2)[1],res=800)
display(img2, method="raster")
coords_sub2 <- coords_sub %>% filter(slice == "BIN1-5-a1")
...
coords_spots <- coords_sub %>%
  ...
  mutate(imagerow = ifelse(slice == "BIN1-5-a1", imagerow-top_cutoff, imagerow)) 
