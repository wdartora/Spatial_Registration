...
# --- Project setup ---
project_root <- "W:/Project/MPI_Registration/Proj01"
setwd(project_root)

# Function for Load10X (2 slices)
source(file.path("scripts", "utils", "backup_functions.R"))

# --- User-defined input ---
slide_name <- "ABCA7-5-a1-7_BIN1-5-a3-8"  # define the two slices separated by underscore
slices <- strsplit(slide_name, "_")[[1]]
sliceA <- slices[1]
sliceB <- slices[2]
...
# Path to the folder containing the Visium data (Space Ranger output)
...
# --- Load Visium data and plot to determine split point ---
se <- Load10X_Spatial_2(data.dir = data_dir, assay = "Spatial")
SpatialDimPlot(se) + NoLegend()

# Add x and y coordinates to spot metadata
...
message("Now open a regular R terminal and run: source('scripts/02_generate_landmarks.R')")
